...
- **Formatting the source** — a toolbar, the keys behind it, and one dialog that lists both. The transforms are **pure functions over a document and a selection, returning an edit rather than a document** — `{ changes, selection }` in the original text's coordinates, which is the shape the image insertion already used. Returning the whole document would be simpler to test and wrong to ship: a document carrying a few hundred kilobytes of base64 would be rewritten end to end to add two asterisks, kept twice over in the undo history, and have every image fold rebuilt on the way past. It also keeps CodeMirror out of it entirely, so the whole thing runs in the `lib` project in milliseconds — where the awkward cases are the tests. Markers are recognised **from outside the selection** as well as inside, so pressing the key twice is a round trip rather than `****bold****`; the space a double-click takes with the word is given back, because CommonMark will not close emphasis on one; and a block prefix crosses **every line the selection touches** and comes off only when it is on all of them. The earlier project did none of those three — it read one line, only ever replaced one marker with another, and looked at nothing but the selected text. Quotes stack in front of whatever the line already is rather than competing with it, since `> - item` is an ordinary thing to want.
  **One table declares every action** — its label, its group, its key and the transform it runs — and the keymap, the toolbar and the dialog all read it, for the same reason the option unions are derived from their option lists: three lists is three places to forget one, and every way that fails is silent. **Enter and Backspace are not ours.** `markdown()` installs `insertNewlineContinueMarkup` and `deleteMarkupBackward`, which is where continuing a list, leaving one, renumbering it and taking markup off a line a character at a time all come from — the earlier project had them too, without noticing. One flag is turned over: by default, Enter on an empty item of a tight list inserts a blank line and keeps the marker, so it takes **three** presses to get out of a list where every editor anyone has used takes one. `nonTightLists: false`, and a loose list is still one blank line away from being typed by hand.
  The toolbar is **one tab stop with arrow keys inside it**, on the same reasoning that put the skip link in M10, and a click never takes focus out of the editor — so the selection it is about to act on is still there when it acts, and the reader is still typing where they were afterwards. Below the breakpoint it is one row that scrolls sideways, which is what an editor on a phone does; the earlier project hid it entirely below 768px, on the one device with no drag-and-drop and an awkward paste. The button that opens the shortcuts dialog is pinned outside that strip, because the way out of not knowing what the bar does is the one thing that must never scroll off the end of it. Two bindings are taken from somebody: `Mod-k` from `deleteLine`, which still has `Mod-Shift-k`, and `Mod-i` from autocompletion. `Mod-e` is inline code as it is in most editors, and it is checked rather than assumed — Chrome hands it through, and Safari keeps it for "Use Selection for Find", where the button is the way to it. `Mod-s` is bound to nothing on purpose: the document saves itself, and unbound the key offers Chrome's "Save page" instead. Underline is cut — it is not CommonMark, we drop raw HTML, and a file that only looks right in our own renderer is the opposite of what the `.md` export is for. `Mod-k` reads the clipboard for a URL, and every way that can be refused ends the same way: the link is inserted with an empty destination, which is what the reader was about to type anyway.
...
- [x] **M11** — Editor toolbar, shortcuts and the dialog that lists them (#12)
